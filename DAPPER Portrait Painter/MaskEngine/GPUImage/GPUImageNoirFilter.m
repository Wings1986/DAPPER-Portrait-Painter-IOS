#import "GPUImageNoirFilter.h"

NSString *const kGPUImageNoirFragmentShaderString = SHADER_STRING(
...
void main(void){
    vec2 vPos=-1.0+2.0*textureCoordinate.xy/resolution.xy;
    ...
    if (f<maxd){
        ...
        if (mod(textureCoordinate.y,2.0) < 1.0) gl_FragColor = mix( gl_FragColor , vec4(0.0), 0.5 ) ;
	}

	// bloom
	vec2 position = textureCoordinate.xy / resolution.xy;

	float aspect = resolution.x/resolution.y;
	float dx = 1.0/resolution.x;
	float dy = dx * aspect;

	vec4 v0 = texture2D( inputImageTexture, position );
	vec4 v1 = texture2D( inputImageTexture, mod ( position + vec2( 0.0, dy ), 1.0 ) );
	vec4 v2 = texture2D( inputImageTexture, mod ( position + vec2( dx, 0.0 ), 1.0 ) );
	vec4 v3 = texture2D( inputImageTexture, mod ( position + vec2( 0.0, -dy ), 1.0 ) );
	vec4 v4 = texture2D( inputImageTexture, mod ( position + vec2( -dx, 0.0 ), 1.0 ) );
	vec4 v5 = texture2D( inputImageTexture, mod ( position + vec2( dx, dy ), 1.0 ) );
	vec4 v6 = texture2D( inputImageTexture, mod ( position + vec2( -dx, -dy ), 1.0 ) );
	vec4 v7 = texture2D( inputImageTexture, mod ( position + vec2( dx, -dy ), 1.0 ) );
	vec4 v8 = texture2D( inputImageTexture, mod ( position + vec2( -dx, dy ), 1.0 ) );
...
@implementation GPUImageNoirFilter
...
- (id)init;
{
    if (!(self = [super initWithFragmentShaderFromString:kGPUImageNoirFragmentShaderString]))
    {
		return nil;
    }
    ...
    mouseUniform = [filterProgram uniformIndex:@"mouse"];
...
- (void)setupFilterForSize:(CGSize)filterFrameSize
{
    runSynchronouslyOnVideoProcessingQueue(^{
        ...
        if (GPUImageRotationSwapsWidthAndHeight(inputRotation))
            glUniform2f(resolutionUniform, filterFrameSize.height, filterFrameSize.width);
        else
            glUniform2f(resolutionUniform, filterFrameSize.width, filterFrameSize.height);
        glUniform2f(mouseUniform, 0.0f, 0.0f);
    });
}
...
{
    if (programIndex == 0)
        glUniform1f(timeUniform, (float)[[NSDate date] timeIntervalSince1970] / 3600.f);
}
