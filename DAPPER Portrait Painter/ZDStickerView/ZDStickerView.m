...
@interface ZDStickerView ()
{
    ...
    UITapGestureRecognizer * customTapGesture;
}
...
@property (strong, nonatomic) UIImageView *doneControl;
...
@synthesize resizingControl, deleteControl, customControl, doneControl;
...
-(void)singleOKTap:(UIPanGestureRecognizer *)recognizer
{
    if([_delegate respondsToSelector:@selector(stickerViewDidClose:)]) {
        [_delegate stickerViewDidClose:self];
    }
    
    if (NO == self.preventsDone) {
    ...
    else if ([recognizer state] == UIGestureRecognizerStateChanged)
    {
        if (self.bounds.size.width < minWidth || self.bounds.size.height < minHeight)
        {
            ...
            customControl.frame =CGRectMake(self.bounds.size.width-kZDStickerViewControlSize,
                                              ...
                                              kZDStickerViewControlSize,
                                              kZDStickerViewControlSize);
            doneControl.frame = CGRectMake(0,
                                             self.bounds.size.height-kZDStickerViewControlSize,
                                             kZDStickerViewControlSize, kZDStickerViewControlSize);
...
//    [self addGestureRecognizer:longpress];
    ...
    doneControl = [[UIImageView alloc]initWithFrame:CGRectMake(0, self.frame.size.height-kZDStickerViewControlSize,
                                                                 kZDStickerViewControlSize, kZDStickerViewControlSize)];
    doneControl.backgroundColor = [UIColor clearColor];
    doneControl.image = [UIImage imageNamed:@"ZDStickerView.bundle/ZDBtn3.png" ];
    doneControl.userInteractionEnabled = YES;
    doneTap = [[UITapGestureRecognizer alloc]
                 initWithTarget:self
                 action:@selector(singleOKTap:)];
//    [doneControl addGestureRecognizer:doneTap];
//    [self addSubview:doneControl];
    [self addGestureRecognizer:doneTap];
    ...
    customTapGesture = [[UITapGestureRecognizer alloc]
                                          initWithTarget:self
...
- (void)setContentView:(UIView *)newContentView {
    ...
    [self bringSubviewToFront:doneControl];
}
...
- (void)setButton:(ZDSTICKERVIEW_BUTTONS)type image:(UIImage*)image
{
    switch (type) {
        ...
        case ZDSTICKERVIEW_BUTTON_CUSTOM:
            ...
            break;
        case ZDSTICKERVIEW_BUTTON_DONE:
            doneControl.image = image;
            break;
...
- (void) hideGesture
{
    [deleteControl removeGestureRecognizer:singleTap];
    [resizingControl removeGestureRecognizer:panResizeGesture];
    [customControl removeGestureRecognizer:customTapGesture];
//    [doneControl removeGestureRecognizer:doneTap];
    [self removeGestureRecognizer:doneTap];
